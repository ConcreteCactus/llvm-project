...
enum AccessKind : uint8_t {
    AkRead = 0, AkWrite, AkUncheckedRead, AkUncheckedWrite, AkLast
};
...
protected:
  ...
  bool VisitUnaryExprOrTypeTraitExpr(UnaryExprOrTypeTraitExpr* UE) { return false; }
...
private:
  ...
  // Accesses that are inside functions or default parameters are not checked by
...
void ConflictingGlobalAccessesCheck::storeOptions(ClangTidyOptions::OptionMap& Opts)
{
...
void ConflictingGlobalAccessesCheck::registerMatchers(MatchFinder *Finder) {
  ...
  ast_matchers::internal::Matcher<BinaryOperator> BinaryOperatorMatcher =
        unsequencedBinaryOperator(LangStd);
  ...
  if (!LangStd->isCPlusPlus17()) {
      Finder->addMatcher(
          stmt(traverse(TK_AsIs, arraySubscriptExpr().bind("gw"))), this);
  }
...
void ConflictingGlobalAccessesCheck::check(const MatchFinder::MatchResult &Result) {
...
void GlobalRWVisitor::startTraversal(const Expr *E, bool StartUnchecked) {
  ...
  // not checked multiple times. If this weren't the case, we would get infinite
...
void GlobalRWVisitor::traverseFunctionsToBeChecked() {
  ...
  for (size_t I = 0; I < FunctionsToBeChecked.size(); ++I) {
    ...
    // Look at the function parameters as well. They get destroyed at the end of
    // the scope.
    ...
    // The hasBody check should happen before we add the function to the array.
    assert(Func->hasBody());
...
void GlobalRWVisitor::checkFunctionLater(const FunctionDecl* FD) {
  if (!FD->hasBody()) {
    return;
  }
...
void GlobalRWVisitor::checkDestructorLater(const CXXRecordDecl* D) {
    ...
    for (const FieldDecl* F : D->fields()) {
        ...
        const CXXRecordDecl* FieldRecordDecl = 
...
bool GlobalRWVisitor::VisitCXXDeleteExpr(CXXDeleteExpr *DE) {
...
void GlobalRWVisitor::addGlobal(DeclarationName Name, SourceLocation Loc,
  ...
  AccessKind Access = (IsGloballyUnchecked || IsUnchecked)
